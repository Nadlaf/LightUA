...
interface ScheduleResultProps {
  scheduleData: ScheduleResultData | null;
}

const ScheduleResultStyles = () => (
  <style>{`
      .empty-card {
...
const ScheduleResult = ({ scheduleData }: ScheduleResultProps) => {
      ...
      <div className="card list-card">
        ...
        <div className="intervals-list">
          {timeline.map((interval, index) => {
            ...
            return (
              <div key={`${interval.start}-${interval.end}-${index}`} className={`interval-row ${isOff ? 'row-off' : 'row-on'}`}>
